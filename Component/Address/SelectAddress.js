...
export default function SelectAddress({navigation, route}){
    ...
    useEffect(() => {
        ...
        const Load_address_data = async () => {
            ...
            try{
                ...
                let Fetch_address_url = URL.RequestAPI ; 
                let Fetch_address_data = {
                    "Check_status": "Fetch_address", 
                    "Table_name": Table_name
                }; 
                let Fetch_address_option = {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
                    body: JSON.stringify(Fetch_address_data)
                }; 

                let Fetch_address_request = await fetch(Fetch_address_url, Fetch_address_option); 
                let Fetch_address_response = await Fetch_address_request.json() ; 

                if (Fetch_address_response.Status == "Fetch"){
                    setAddressData([...Fetch_address_response.Address]) ; 
                }
    ...
    const Delete_address_Handler = async (address_id) => {
        ...
        try{
            ...
            let Delete_address_url = URL.RequestAPI ; 
            let Delete_address_data = {
                "Check_status": "Delete_address", 
                "Table_name": Table_name, 
                "Delete_address_id": address_id
            } ;
            let Delete_address_option = {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify(Delete_address_data)
            }; 

            let Delete_address_request = await fetch(Delete_address_url, Delete_address_option); 
            let Delete_address_response = await Delete_address_request.json() ; 
            
            if (Delete_address_response.Status == "Delete"){
                ...
                for (let i = 0; i<addressData.length; i++){
                    if (addressData[i].Option == address_id){
                ...
                <ScrollView>
                    ...
                    {addressData.length>0?<>
                        ...
                        <Text style={{fontFamily:"Ubuntu", fontSize:19, width:"90%", 
                            marginLeft:"auto", marginRight:"auto", marginTop:10}}>Not available any address</Text>
                    </>}
